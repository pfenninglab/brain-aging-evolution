...
## Load ArchR Project
setwd(PROJDIR1)
proj <- loadArchRProject(path = PROJDIR1, force = FALSE, showLogo = FALSE)
...
proj = saveArchRProject(ArchRProj = proj, outputDirectory= PROJDIR1)
...
proj <- addGroupCoverages(proj, groupBy = "predicted.id.y", sampleLabels = 'id',
...
proj <- addReproduciblePeakSet(proj, groupBy = "predicted.id.y",
